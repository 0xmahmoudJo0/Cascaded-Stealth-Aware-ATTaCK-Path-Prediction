...
class GRUPredictor(TorchBackedPredictor):
    def __init__(
        self,
    ...
    def _build_model(self):  # pragma: no cover - optional dependency
        """Construct the GRU classifier module if PyTorch is available."""
        ...
        class _GRU(torch.nn.Module):
            def __init__(self, emb, gru_module, hidden_size, dropout, vocab):
                super().__init__()
                self.embedding = emb
                self.gru = gru_module
        ...
        return _GRU(embedding, gru, self.config.hidden_size, self.config.dropout, len(self.technique_ids))
...
__all__ = ["GRUPredictor", "GRUConfig"]
