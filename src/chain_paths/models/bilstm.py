"""BiLSTM-based sequence predictor (with graceful CPU fallback)."""
...
class BiLSTMPredictor(TorchBackedPredictor):
    """Wrapper that would host a BiLSTM when PyTorch is available."""
    ...
    def __init__(
        ...
        *,
        ...
        # Embeddings are accepted for interface parity but not used in this lightweight stub.
    ...
    def _build_model(self):  # pragma: no cover - optional dependency
        """Construct the BiLSTM classifier module if PyTorch is available."""
        ...
        class _BiLSTM(torch.nn.Module):
            def __init__(self, emb, lstm_module, hidden_size, dropout, vocab):
                ...
                self.output = torch.nn.Linear(hidden_size * 2, vocab)
        ...
        return _BiLSTM(embedding, lstm, self.config.hidden_size, self.config.dropout, len(self.technique_ids))
...
__all__ = ["BiLSTMPredictor", "BiLSTMConfig"]
